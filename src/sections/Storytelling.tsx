...
export const Storytelling = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-32 -right-32 w-64 h-64 sm:w-80 sm:h-80 bg-luxury-brand/5 rounded-full filter blur-3xl pointer-events-none"></div>

      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-center">
          {/* Emotional Copy Right Column - Mobile Order 1 / Desktop Order 2 */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            className="lg:col-span-7 text-right order-1 lg:order-2"
          ...
          >
            ...
            >
              <Quote className="w-6 h-6 sm:w-8 sm:h-8 text-luxury-brand/10 absolute -top-2.5 -left-1.5 transform flip-x" />
          ...
          {/* Editorial Visual Frame Left - Mobile Order 2 / Desktop Order 1 */}
          <motion.div
            initial={{ opacity: 0, scale: 0.98 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
            className="lg:col-span-5 relative group order-2 mt-4 lg:mt-0"
            id="storytelling-image-wrapper"
          >
            {/* Soft decorative shadow framing */}
            <div className="absolute -inset-1.5 bg-linear-to-tr from-luxury-brand/5 to-transparent rounded-2xl sm:rounded-3xl -rotate-1 scale-101 filter blur-xs pointer-events-none"></div>
            ...
            >
              <img
                ...
                className="w-full h-full object-cover transition-transform duration-[6s] group-hover:scale-103"
            ...
            <div
              className="absolute -bottom-3 -left-3 sm:-bottom-6 sm:-right-6 bg-[#141416]/95 backdrop-blur-md border border-luxury-brand/15 rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xl max-w-60 sm:max-w-60 text-right transition-transform duration-300 group-hover:-translate-y-0.5"
